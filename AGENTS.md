...
Каждая версия живёт под своим префиксом: v1 под `/v1`, v2 под `/v2`. Пути
внутри документа OpenAPI префикса не знают, его навешивает glue при
регистрации, поэтому документу нужен `servers` с тем же префиксом. Подставляет
его `src/plugins/docs.ts` при отдаче `/v1/openapi.json` и `/v2/openapi.json`.
Без этого собранный по документу клиент стучится в корень, где маршрутов нет.
...
- **Тесты**: раскладка по маршрутам (`test/routes/*.test.ts`), всё остальное —
  старт, документы, формат ошибок, health и metrics — в `test/app.test.ts`.
  Запросы идут только через `app.inject()`. Приложение приходит фикстурой
  `createTest()` из `test/helper.ts`: поднимается один раз на файл, а каждый
  тест выполняется в транзакции и откатывается. Из-за фикстуры отключено
  правило `vitest/no-standalone-expect` — оно признаёт тестом только глобальный
  `test`, а не полученный из `test.extend`.
- **Ассерты без сообщений**: `expect` печатает полученное значение сам. Если
  данных в диффе не хватает, проверяется составное значение
  (`expectStatus` сверяет статус вместе с телом), а не дописывается текст.
