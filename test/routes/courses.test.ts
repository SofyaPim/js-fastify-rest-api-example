...
test("get courses", async ({ app }) => {
  const res = await app.inject({ url: "/v1/courses" });
...
test("get courses/:id", async ({ app }) => {
  ...
  const res = await app.inject({ url: `/v1/courses/${course.id}` });
  expectStatus(res, 200);
...
test("writing to courses requires a token, reading does not", async ({ app }) => {
  const writes = [
    { method: "post", url: "/v1/courses", body: { name: "Course", description: "Text" } },
    { method: "put", url: "/v1/courses/1", body: { name: "Renamed" } },
    { method: "delete", url: "/v1/courses/1" },
  ] as const;
  ...
  const course = await firstCourse(app);
  expectStatus(await app.inject({ url: "/v1/courses" }), 200);
  expectStatus(await app.inject({ url: `/v1/courses/${course.id}` }), 200);
});
...
test("operations on a missing course answer 404", async ({ app }) => {
  ...
  const cases = [
    { method: "get", url: `/v1/courses/${MISSING_ID}` },
    { method: "put", url: `/v1/courses/${MISSING_ID}`, body: { name: "Nobody" } },
    { method: "delete", url: `/v1/courses/${MISSING_ID}` },
  ] as const;
...
test("deleting a course removes its lessons", async ({ app }) => {
  ...
  const res = await app.inject({
    method: "delete",
    url: `/v1/courses/${lesson.courseId}`,
    headers: { ...authHeader },
  });
  expectStatus(res, 204);
...
test("a course carries an ETag and honours If-None-Match", async ({ app }) => {
  ...
  const first = await app.inject({ url: `/v1/courses/${course.id}` });
...
test("a stale If-Match is rejected instead of overwriting", async ({ app }) => {
  ...
  const fetched = await app.inject({ url: `/v1/courses/${course.id}` });
...
test("a list reports where the client is and how much there is", async ({ app }) => {
  ...
  const first = await app.inject({ url: "/v1/courses?page=1&perPage=3" });
  expectStatus(first, 200);
  ...
  const last = await app.inject({ url: "/v1/courses?page=3&perPage=3" });
...
test("a page past the end is empty but still reports the total", async ({ app }) => {
  const res = await app.inject({ url: "/v1/courses?page=99&perPage=10" });
  expectStatus(res, 200);
  const page = res.json();
